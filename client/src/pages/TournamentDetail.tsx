...
const MatchRow: React.FC<{ match: Match; favoriteTeamIds: string[]; startingMatch: string | null; onStart: (matchId: string) => void; onManualStart: (match: Match) => void }> = ({ match, favoriteTeamIds, startingMatch, onStart, onManualStart }) => {
  ...
  return <div className="border rounded-lg p-4 flex justify-between items-center"><div className="flex-1 min-w-0"><h4 className="font-semibold mb-1 flex items-center gap-2 min-w-0"><TeamNameWithFlag team={match.homeTeam} className={homeFavorite ? 'text-yellow-700' : 'text-gray-900'} flagClassName="w-5 h-4 flex-shrink-0" /><span className="text-gray-400 flex-shrink-0">vs</span><TeamNameWithFlag team={match.awayTeam} className={awayFavorite ? 'text-yellow-700' : 'text-gray-900'} flagClassName="w-5 h-4 flex-shrink-0" /></h4><p className="text-sm text-gray-600">第 {match.round} 轮 - {match.status === 'scheduled' ? '待进行' : match.status === 'in_progress' ? '进行中' : '已结束'}{match.resultMode === 'manual' && <span className="ml-2 text-blue-700">手工计算</span>}</p></div><div className="flex items-center space-x-4">{match.status !== 'scheduled' && <span className="font-bold text-xl">{match.homeScore ?? 0} - {match.awayScore ?? 0}{hasPenaltyResult(match) && <span className="ml-2 text-sm text-gray-600">点球 {match.homePenaltyScore} - {match.awayPenaltyScore}</span>}</span>}<div className="flex space-x-2">{match.status === 'scheduled' && <><button onClick={() => onStart(match.id)} disabled={startingMatch === match.id} className="bg-green-600 text-white px-3 py-1 rounded text-sm hover:bg-green-700 disabled:opacity-50">{startingMatch === match.id ? '开始中...' : '自动进行'}</button><button onClick={() => onManualStart(match)} className="bg-amber-600 text-white px-3 py-1 rounded text-sm hover:bg-amber-700">手动进行</button></>}<Link to={`/matches/${match.id}`} className="bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700">{match.status === 'scheduled' ? '查看详情' : '查看统计'}</Link></div></div></div>;
};
